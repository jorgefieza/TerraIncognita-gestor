...
import { format, parseISO, set, addDays, areIntervalsOverlapping, subDays } from 'date-fns';
import { pt } from 'date-fns/locale';
import { AlertTriangleIcon, TrashIcon, XCircleIcon, CheckCircleIcon, UsersIcon, XMarkIcon } from './Icons';
...
const EventModal = () => {
    ...
    const [eventDepartment, setEventDepartment] = useState(department);
    ...
    const [financialDetails, setFinancialDetails] = useState({ grossValue: '', commissionType: '%', commissionValue: '', iva: '23' });
    ...
    useEffect(() => {
        if (isEventModalOpen) {
            ...
            setFinancialDetails(initialEvent.financialDetails || { grossValue: '', commissionType: '%', commissionValue: '', iva: '23' });
    ...
    const handleFinancialChange = (e) => { const { name, value } = e.target; setFinancialDetails(prev => ({ ...prev, [name]: value })); };
    ...
    const handleCancelEvent = () => { if (selectedEvent && permissions.canCancelEvent(currentUser, selectedEvent)) { if (window.confirm(`Tem a certeza que quer CANCELAR o evento "${selectedEvent.title}"?`)) { eventService.save({ ...selectedEvent, status: 'Cancelado' }, currentUser); closeEventModal(); } } };
    ...
    const handleResumeEvent = () => { if (selectedEvent && permissions.canCancelEvent(currentUser, selectedEvent)) { if (window.confirm(`Tem a certeza que quer RETOMAR o evento "${selectedEvent.title}"? O status será alterado para Standby.`)) { eventService.save({ ...selectedEvent, status: 'Standby' }, currentUser); closeEventModal(); } } };
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                            <div className="space-y-4">
                                <div className="grid grid-cols-2 gap-4">
                                    <div><label className="block text-sm font-medium text-gray-700 mb-1">Início</label><input type="date" value={startDate} onChange={e => setStartDate(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-md" /><select value={startTime} onChange={e => setStartTime(e.target.value)} className="w-full mt-2 px-3 py-2 border border-gray-300 rounded-md bg-white">{timeOptions.map(time => <option key={time} value={time}>{time}</option>)}</select></div>
                                    <div><label className="block text-sm font-medium text-gray-700 mb-1">Fim</label><input type="date" value={endDate} onChange={e => setEndDate(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-md" /><select value={endTime} onChange={e => setEndTime(e.target.value)} className="w-full mt-2 px-3 py-2 border border-gray-300 rounded-md bg-white">{timeOptions.map(time => <option key={time} value={time}>{time}</option>)}</select></div>
                                </div>
                                <div className="grid grid-cols-2 gap-4">
                                    <div><label className="block text-sm font-medium text-gray-700 mb-1">Ponto de Embarque</label><select value={boardingPointId} onChange={e => setBoardingPointId(e.target.value)} className="w-full mt-1 px-3 py-2 border border-gray-300 rounded-md bg-white"><option value="">Padrão (Doca de St Amaro)</option>{allDocks.map(dock => <option key={dock.id} value={dock.id}>{dock.name}</option>)}</select></div>
                                    <div><label className="block text-sm font-medium text-gray-700 mb-1">Ponto de Desembarque</label><select value={disembarkingPointId} onChange={e => setDisembarkingPointId(e.target.value)} className="w-full mt-1 px-3 py-2 border border-gray-300 rounded-md bg-white"><option value="">Padrão (Doca de St Amaro)</option>{allDocks.map(dock => <option key={dock.id} value={dock.id}>{dock.name}</option>)}</select></div>
                                </div>
                                <div className="relative" ref={equipmentSearchRef}><label className="block text-sm font-medium text-gray-700 mb-1">Equipamentos</label><input type="text" placeholder="Digite um termo" value={equipmentSearch} onChange={e => setEquipmentSearch(e.target.value)} onFocus={() => setEquipmentSearchOpen(true)} className="w-full px-3 py-2 border border-gray-300 rounded-md mb-2" />
                                ...
                                <div className="relative" ref={professionalSearchRef}><label className="block text-sm font-medium text-gray-700 mb-1">Profissionais</label><input type="text" placeholder="Digite um nome ou habilidade" value={professionalSearch} onChange={e => setProfessionalSearch(e.target.value)} onFocus={() => setProfessionalSearchOpen(true)} className="w-full px-3 py-2 border border-gray-300 rounded-md mb-2" />
                                 ...
                                 <div><label className="block text-sm font-medium text-gray-700 mb-1">Notas do Evento</label><textarea name="note" value={note} onChange={(e) => setNote(e.target.value)} rows="2" className="w-full px-3 py-2 border border-gray-300 rounded-md" placeholder="Instruções especiais..."></textarea></div>
                            </div>
                            <div className="space-y-4">
                                <div><label className="block text-sm font-medium text-gray-700 mb-1">Departamento</label>
                                    <select name="department" value={eventDepartment} onChange={e => setEventDepartment(e.target.value)} className="w-full mt-1 px-3 py-2 border border-gray-300 rounded-md bg-white"><option value="Turismo">Turismo</option><option value="Comercial">Comercial</option><option value="Escola">Escola</option></select>
                                </div>
                                {eventDepartment === 'Comercial' && (
                                    <div className="relative" ref={clientSearchRef}>
                                        <label className="block text-sm font-medium text-gray-700 mb-1">Cliente</label>
                                        <input type="text" placeholder="Digite para procurar um cliente..." value={clientSearch} onChange={e => setClientSearch(e.target.value)} onFocus={() => setIsClientSearchOpen(true)} className="w-full px-3 py-2 border border-gray-300 rounded-md" />
                                ...
                                {(eventDepartment === 'Turismo' || eventDepartment === 'Escola') && (
                                    <div className="relative" ref={productSearchRef}>
                                        ...
                                        <input type="text" placeholder="Digite para procurar um produto..." value={productSearch} onChange={e => setProductSearch(e.target.value)} onFocus={() => setIsProductSearchOpen(true)} className="w-full px-3 py-2 border border-gray-300 rounded-md" />
                                ...
                                {showFinancials && (<div className="p-3 border border-gray-200 rounded-md space-y-3 bg-gray-50"><h4 className="text-sm font-bold text-gray-600">Detalhes Financeiros</h4><div><label className="block text-xs font-medium text-gray-600">Valor Bruto (€)</label><input type="number" name="grossValue" value={financialDetails.grossValue} onChange={handleFinancialChange} className="w-full mt-1 px-2 py-1 border border-gray-300 rounded-md text-sm" /></div><div className="grid grid-cols-2 gap-3"><div><label className="block text-xs font-medium text-gray-600">Comissão</label><div className="flex"><input type="number" name="commissionValue" value={financialDetails.commissionValue} onChange={handleFinancialChange} className="w-full mt-1 px-2 py-1 border-r-0 border-gray-300 rounded-l-md text-sm" /><select name="commissionType" value={financialDetails.commissionType} onChange={handleFinancialChange} className="mt-1 border border-gray-300 rounded-r-md bg-gray-100 text-sm"><option value="%">%</option><option value="€">€</option></select></div></div><div><label className="block text-xs font-medium text-gray-600">IVA (%)</label><input type="number" name="iva" value={financialDetails.iva} onChange={handleFinancialChange} className="w-full mt-1 px-2 py-1 border border-gray-300 rounded-md text-sm" /></div></div></div>)}
                            </div>
                        </div>
                    </fieldset>
                    <div className="flex justify-between items-center mt-8 pt-4 border-t print-hidden">
                        ...
                        <div>
                            {selectedEvent?.status === 'Cancelado' ? (
                                permissions.canCancelEvent(currentUser, selectedEvent) && !selectedEvent.autoCancelled && (
                                    <button onClick={handleResumeEvent} title="Retomar Evento" className="flex items-center p-2 bg-red-600 text-white rounded-md hover:bg-red-700">
                                        <CheckCircleIcon className="w-6 h-6"/>
                                    </button>
                                )
                            ) : (
                                selectedEvent && permissions.canCancelEvent(currentUser, selectedEvent) && (
                                    <button onClick={handleCancelEvent} title="Cancelar Evento" className="flex items-center p-2 bg-red-600 text-white rounded-md hover:bg-red-700">
                        ...
                        <div>
                            {selectedEvent?.status === 'Cancelado' ? (
                                permissions.canDeleteEvent && (
                                    <button onClick={handleDelete} title="Excluir Permanentemente" className="text-red-600 hover:text-red-800 font-medium p-2 rounded-md hover:bg-red-50">
