...
const EventModal = () => {
    ...
    const handleSave = () => {
        ...
        if (selectedEvent?.status === 'Cancelado') {
            ...
            const allResourcesConfirmed = (hasEquipment || hasProfessionals) && 
                                          selectedEquipment.every(e => e.confirmed) && 
                                          selectedProfessionals.every(p => p.confirmed);
            const isPaidIfRequired = eventDepartment !== 'Comercial' || (eventDepartment === 'Comercial' && financialDetails.isPaid);
            if (hasEquipment && selectedProfessionals.length < requiredProfessionals) {
                 finalStatus = 'Standby';
            } else {
                 finalStatus = allResourcesConfirmed && isPaidIfRequired ? 'Confirmado' : 'Standby';
            }
        }
        const eventData = { title: finalTitle, department: eventDepartment, status: finalStatus, start: startDateTime.toISOString(), end: endDateTime.toISOString(), equipment: selectedEquipment, professionals: selectedProfessionals, clientId: selectedClient?.id || null, productId: selectedProductId || null, agencyId: selectedAgencyId || null, note, type: 'Evento Padrão', financialDetails: showFinancials ? financialDetails : null, boardingPointId, disembarkingPointId };
    ...
    const addResource = (resource, type) => {
        ...
        if (type === 'equipment') {
            if (!selectedEquipment.some(e => e.name === resource.name)) setSelectedEquipment(prev => [...prev, { name: resource.name, confirmed: false, conflict: availability.status !== 'available' }]);
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                            <div className="space-y-4">
                                ...
                                <div><label className="block text-sm font-medium text-gray-700 mb-1">Departamento</label>
                                    <select name="department" value={eventDepartment} onChange={e => setEventDepartment(e.target.value)} disabled={role !== 'diretor'} className={role !== 'diretor' ? 'bg-gray-100 cursor-not-allowed' : ''}><option value="Turismo">Turismo</option><option value="Comercial">Comercial</option><option value="Escola">Escola</option></select>
                                </div>
                                ...
                                {eventDepartment === 'Comercial' && (
                                  <>
                                    ...
                                    <div className="relative" ref={agencySearchRef}>
                                        ...
                                        <input type="text" placeholder="Procurar agência..." value={agencySearch} onChange={e => setAgencySearch(e.target.value)} onFocus={() => setIsAgencySearchOpen(true)} />
                                        {isAgencySearchOpen && (<div className="absolute z-30 w-full mt-1 bg-white border border-gray-300 rounded-md shadow-lg max-h-40 overflow-y-auto">{(suggestedAgencies || []).map(a => <button key={a.id} onClick={() => { setSelectedAgencyId(a.id); setAgencySearch(a.name); setIsAgencySearchOpen(false); }} className="w-full text-left p-2 hover:bg-gray-50">{a.name}</button>)}</div>)}
                                        {selectedAgencyId && !isAgencySearchOpen && <button onClick={() => {setSelectedAgencyId(''); setAgencySearch('');}} className="absolute top-7 right-2 text-gray-400 hover:text-red-500"><XCircleIcon /></button>}
                                    </div>
                                  </>
                                )}
                            ...
                            <div className="space-y-4">
                                <div className="relative" ref={equipmentSearchRef}>
                                    ...
                                    <input type="text" placeholder="Digite um termo" value={equipmentSearch} onChange={e => setEquipmentSearch(e.target.value)} onFocus={() => setEquipmentSearchOpen(true)} className="mb-2" />
                                    {isEquipmentSearchOpen && (
                                        <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-md shadow-lg max-h-40 overflow-y-auto">
                                            {suggestedEquipment.map(e => { 
                                                const isSelected = selectedEquipment.some(se => se.name === e.name);
                                                // Bloqueia apenas se for 'unavailable' (conflito severo)
                                                const isDisabled = isSelected || e.availability.status === 'unavailable';
                                                return (
                                                    <button key={e.id} onClick={() => addResource(e, 'equipment')} disabled={isDisabled} className={`w-full text-left p-2 flex justify-between items-center ${isDisabled ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : 'hover:bg-gray-50'}`}>
                                                        <div className="flex flex-col">
                                                            <span>{e.name}</span>
                                                            {e.minProfessionals > 0 && <span className="text-xs text-gray-500 flex items-center"><UsersIcon className="w-3 h-3 mr-1" /> Mín. {e.minProfessionals}</span>}
                                                        </div>
                                                        {/* Mostra o aviso para qualquer tipo de conflito */}
                                                        {e.availability.status !== 'available' && <span title={e.availability.reason}><AlertTriangleIcon className={`w-4 h-4 ${e.availability.status === 'unavailable' ? 'text-red-600' : 'text-orange-500'}`} /></span>}
                                                    </button>
                                                );
                                            })}
                                        </div>
                                ...
                                <div className="relative" ref={professionalSearchRef}>
                                    ...
                                    <input type="text" placeholder="Digite um nome ou habilidade" value={professionalSearch} onChange={e => setProfessionalSearch(e.target.value)} onFocus={() => setProfessionalSearchOpen(true)} className="mb-2" />
                                    {isProfessionalSearchOpen && (
                                        <div className="absolute z-20 w-full mt-1 bg-white border border-gray-300 rounded-md shadow-lg max-h-40 overflow-y-auto">
                                            {suggestedProfessionals.map(p => { 
                                                const isSelected = selectedProfessionals.some(sp => sp.name === p.name);
                                                const isDisabled = isSelected || p.availability.status === 'unavailable';
                                                return (
                                                    <button key={p.idWithSkill} onClick={() => addResource(p, 'professional')} disabled={isDisabled} className={`w-full text-left p-2 flex justify-between items-center ${isDisabled ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : 'hover:bg-gray-50'}`}>
                                                        <span>{p.displayName}</span>
                                                        {p.availability.status !== 'available' && <span title={p.availability.reason}><AlertTriangleIcon className={`w-4 h-4 ${p.availability.status === 'unavailable' ? 'text-red-600' : 'text-orange-500'}`} /></span>}
                                                    </button>
                                                );
                                            })}
                                        </div>
